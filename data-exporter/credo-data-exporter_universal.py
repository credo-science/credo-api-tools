...
parser.add_argument("--max-chunk-size", "-m", help="Maximum number of events in each file", type=int, default=30000)
...
# Jeśli systemem operacyjnym jest Windows, użyj formatu ścieżki plików dla Windows,
# w przeciwnym razie użyj formatu ścieżki plików dla systemów Unix (Linux, macOS)
...
def prepare_workspace():
    ...
    # Utwórz katalogi dla typów danych, jeśli nie istnieją
    for data_type in ["detections", "pings"]:
        data_dir = FILE_PATH_FORMAT.format(args.dir, data_type)
        if not os.path.exists(data_dir):
            os.makedirs(data_dir)

    # Utwórz plik urządzenia, jeśli nie istnieje, i zapisz w nim identyfikator urządzenia
    device_id_path = FILE_PATH_FORMAT.format(args.dir, "device_id")
